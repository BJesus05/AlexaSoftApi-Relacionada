...
const initDataTable = async () => {
  ...
  dataTable = $("#examplee").DataTable(dataTableOptions);

  $("#motvio").on("input", function () {
    dataTable.column(".motvio:name").search($(this).val()).draw();
    updateFilteredList();
  });
...
const listUsers = async () => {
  try {
    const response = await fetch("http://localhost:4000/motivoscancelacion");
    ...
    users.forEach((user) => {
      content += `
              <tr>
                  ...
                  <td> ${user.motivo} </td>
                  <td>
                    <button class="btn btn-sm btn-primary" data-bs-toggle="modal" onclick="editarMotivo(${user.idMotivo})"><i class="fa-solid fa-pencil"></i></button>
                    <button class="btn btn-sm btn-danger" onclick="confirmDelete(${user.idMotivo})"><i class="fa-solid fa-trash-can"></i></button>
                  </td>
              </tr>`;
    });
    $("#motivo_list").html(content);
  } catch (error) {
    alert(error);
  }
};
